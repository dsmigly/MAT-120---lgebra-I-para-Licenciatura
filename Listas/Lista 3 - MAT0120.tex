...
\begin{solution}
%Suponha que exista apenas uma quantidade finita de primos da forma $3n+2$, digamos, $p_1, p_2, \ldots, p_k.$ Considere $n = p_1 \cdot p_2 \cdot \ldots \cdot p_k - 1.$ Sabemos então que $n > p_k.$ Observe que $n$ possui ao menos um fator primo da $3n+2,$ pois se todos fossem da forma $3n+1,$ 
\textbf{Observação:} O Teorema de Dirichlet afirma que existem infinitos números primos na forma $an + d,$ onde $\mdc(a,d) = 1.$ Uma demonstração similar à feita acima funciona para $an +d $ se e somente se $a \mid d^2 - 1.$
%http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.225.9834&rep=rep1&type=pdf
\end{solution}
\begin{exercicio}
...
\begin{exercicio}
O valor numérico de cada expressão a seguir é primo?

\itensladoalado{2}{
\task[\alt{a}] $2^6 + 3$
\task[\alt{b}] $4^2 + 5^2$
\task[\alt{c}] $47^2 - 37^2 - 23^2$
\task[\alt{d}] $1 + 2^2 + 3^3 + 4^4 + 5^5$
}
\end{exercicio}
\begin{comment}
...
\end{comment}
\begin{exercicio} 
Encontre todos os primos $p$ tais que $7p^2 + 8$ é também um primo.
\end{exercicio}
\begin{solution}
Vamos analisar os possíveis restos da divisão de $7p^2 + 8$ por $3.$ Pelo Algoritmo da Divisão, sabemos que $p = 3k+r,$ onde $k \in \mathbb{Z}$ e $r \in \{0,1,2 \}.$ Analisemos cada caso:
\begin{itemize}
    \item 
\end{itemize}
\end{solution}
...
\begin{exercicio}
Prove que o resto da divisão de qualquer número primo por $30$ é igual a $1$ ou igual a um número primo.
\end{exercicio}
\begin{solution}
Seja $p$ um número primo tal que $p = 30n + r,$ onde $r \in \{0,1, \ldots, 29 \}.$
Como $30 = 2 \cdot 3 \cdot 5,$ então $r$ não pode ser nem  múltiplo de $2$ nem múltiplo de $3$ e nem  múltiplo de $5$. Assim, $r = 1$ ou $r$ é primo.
%https://math.stackexchange.com/questions/398399/about-the-property-of-m-if-n-m-is-co-prime-to-m-then-n-is-prime/398525#398525
\end{solution}
...
\begin{exercicio}\textcolor{Blue}{*}
O \textbf{Postulado de Bertrand} afirma que para todo inteiro $n \ge 1,$ existe um número primo entre $n$ e $2n.$ Use este resultado para provar que existem infinitos números primos que começam com o dígito $1.$ 
\end{exercicio}
\begin{solution}
Observe que, se $p$ é um número primo iniciado em $1,$ então $1 \cdot 10^k < p < 1 \cdot 2 \cdot 10^{k}.$ Assim, tomando $n = 10^k,$ pelo Postulado de Bertrand, para cada $k \ge 1$ existe um número primo entre $1 \cdot 10^k < p < 1 \cdot 2 \cdot 10^{k}.$
\end{solution}
\begin{exercicio}
...
\end{solution}
\begin{exercicio}\textcolor{Blue}{*}
Seja $S_n$ a soma dos $n$ primeiros números primos. Por exemplo, $$S_4 = 2 + 3 + 5 + 7 = 17$$ e $$S_7 = 2 + 3 + 5 + 7 + 11 + 13 + 17 = 58.$$ Prove que existe ao menos um número quadrado perfeito entre $S_n$ e $S_{n+1}.$
\end{exercicio}
\begin{solution}
...
\end{solution}
\begin{exercicio}\textcolor{Blue}{*}
Sejam $p$ e $q$ dois números primos. Prove que $pq + 1$ é um número quadrado perfeito se e somente se $p$ e $q$ forem números primos gêmeos.
\end{exercicio}
\begin{solution}
$(\Rightarrow)$ Se $pq+1$ é um número quadrado perfeito, então existe um $n$ tal que $pq + 1 = n^2,$ e assim
$$pq+1=n^2\Rightarrow pq=(n-1)(n+1).$$
Como $p$ e $q$ são números primos, temos que $p = n - 1$ e $q = n + 1,$ e portanto $q - p = (n+1) - (n-1) = 2.$ Assim, $p$ e $q$ são números primos gêmeos.

$(\Leftarrow)$
Se $p$ e $q$ são números primos gêmeos, então podemos escrever que $p=q+2,$ e então
$$pq+1=(q+2)q = q^2+2q+1=(q+1)^2.$$ 
\end{solution}
\begin{exercicio}
Dizemos que um número primo $p > 2$ é \textit{pitagórico} se existem $a, b \in \mathbb{N}$ tais que $p = a^2 + b^2.$ 
\itens{
\task[\alt{a}] Prove que $p$ é da forma $4n+1,$ e conclua que existem infinitos números primos pitagóricos.
\task[\alt{b}] Prove que todos números primos pitagóricos menores do que 100 podem ser obtidos tomando $a,b \in \{1,2,3,4,5,6,7,8,9\}.$
}
%https://math.stackexchange.com/questions/1502062/primes-written-as-the-sum-of-two-squares?rq=1
\end{exercicio}
\begin{solution}

Os números primos da forma $4n+1$ menores do que $100$ são
\[
5, 13, 17, 29, 37, 41, 53, 61, 73, 89, 97\]
A tabela abaixo mostra que é possível escrever cada um dos números primos acima onde $a,b \in \{1,2,3,4,5,6,7,8,9\}:$
\begin{tabular}{|c|c|c|}
    p & a & b \\
    5 & 1 & 6 \\
    13 & 2 & 3\\
